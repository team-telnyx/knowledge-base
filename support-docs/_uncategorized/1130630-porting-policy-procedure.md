...
* Create a Telnyx Mission Control account and proceed to the [Port Numbers](https://portal.telnyx.com/#/app/numbers/port-numbers) page.
...
* For each port request, we require two documents: an LOA signed by the End Customer where the signer matches what is on file with the existing carrier. Please ensure the LOA lists all numbers porting, the customer information, and either a wet signature or valid signature through Docusign or another similar method. Second, provide the most recent bill copy from the existing carrier.
...
We observe the following US Porting holidays:
...
* All port out requests must be submitted to [lnp@telnyx.com](mailto:lnp@telnyx.com). We do not accept port outs over the phone.
