...
3. Company Registration number if the number belongs to a company / Personal code if the number belongs to a private customer

**Please note**, after the porting request is confirmed (status: FOC confirmed), it will no longer be possible to cancel the port. The number will have to port back to the original carrier after port is fully executed.
