...
2. Please note that FOC date changes within 48 hours of FOC are considered best effort requests, as the losing carrier may still drop the lines.
3. When  submitting  a  port  request,  we ask  that you submit a LOA,  an  invoice, and where possible, a CSR (Customer  Service  Record). A CSR can be added after the port request has been created within the Portal - CSR helps to validate the end user information if the losing carrier rejects the port request for invalid information
4. When creating the port request, you will have the option to assign a connection. We recommend assigning a connection to the TNs so that as soon as the numbers are ported to Telnyx, they will automatically hit the connections that they are assigned to.
