...
*Part 5 of 6 — see also: [Part 1](number-reputation--part-1.md), [Part 2](number-reputation--part-2.md), [Part 3](number-reputation--part-3.md), [Part 4](number-reputation--part-4.md), [Part 6](number-reputation--part-6.md)*
...
Submitting a remediation request does **not** guarantee a number will be removed from any spam or block list. Remediation is a **re-evaluation request** to the call-analytics networks — the outcome may be that the number remains flagged (`requires_review` or `refused`). Even after a number is successfully remediated, calls from it can **still be blocked or labeled** by downstream spam filters and carriers. Those systems are outside Telnyx's control, and a successful remediation does not override them.
