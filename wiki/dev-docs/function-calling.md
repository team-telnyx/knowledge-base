...
summary: Learn how to connect large language models hosted on Telnyx to external tools
  using the chat completions API, including defining functions, letting the model
  choose them, executing them, and returning results to the model.
sources:
- url: https://developers.telnyx.com/docs/inference/functions
updated_at: 2026-08-05T13:46:11Z
---

# Function Calling

Learn how to connect large language models hosted on Telnyx to external tools using the chat completions API, including defining functions, letting the model choose them, executing them, and returning results to the model.

## Introduction

Using the `tools` field, you can enable a language model to choose functions to call. The [Chat Completions API](chat-completions-api.md) does not call the function itself. It will return the arguments you need to execute the function yourself.

Of the open-source language models hosted on Telnyx, `zai-org/GLM-5.2` is especially good at calling functions. While we recommend you start with this model, every model in the API supports the `tools` interface.

## Simple `get_current_weather` example

A popular toy example for function calls is the `get_current_weather` example. The following code defines a function and passes it to the language model via the `tools` field.
...
- `required`: forces the language model to choose a tool
- `none`: forces the language model to NOT choose a tool
- `auto`: lets the language model decide
...
It's nice that the language model wants to execute `get_current_weather`, but how do we actually do that, and incorporate the results back into the interaction? Continuing from the `chat_completion` response in the previous example:
