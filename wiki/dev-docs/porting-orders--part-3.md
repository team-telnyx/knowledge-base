...
*Part 3 of 4 — see also: [Part 1](porting-orders--part-1.md), [Part 2](porting-orders--part-2.md), [Part 4](porting-orders--part-4.md)*
...
Telnyx offers port-out notifications for the following events:
...
If you are adding port-out notification settings in the portal, they will apply for all port orders that you create:
...
For further questions, see the [notification settings support article](https://support.telnyx.com/en/articles/4277896-notification-settings) or contact Telnyx customer support.

### Example webhook notification events

The following examples illustrate the webhook payloads emitted for common port-out lifecycle events.
...
If the gaining carrier requests a FOC date change, the following notification is emitted. For port-out orders, the gaining carrier is allotted a grace period of 10 days, which means the gaining carrier can port the phone number at any point between the FOC date listed and the following 10 days. While most port-out orders are expected to occur on the listed FOC date, there is no guarantee that they will be completed on that day.
