...
*Part 3 of 7 — see also: [Part 1](webrtc-voice-sdks--part-1.md), [Part 2](webrtc-voice-sdks--part-2.md), [Part 4](webrtc-voice-sdks--part-4.md), [Part 5](webrtc-voice-sdks--part-5.md), [Part 6](webrtc-voice-sdks--part-6.md), [Part 7](webrtc-voice-sdks--part-7.md)*
...
## JS SDK Client and Call Lifecycle

While some differences exist between the JS SDK and the mobile SDKs, they follow a similar client lifecycle and call flow. The JS SDK demo app is used here as it is far easier to set up the application (just load up `webrtc.telnyx.com`) and perform debugging using browser tooling.
...
2. Right-click, Inspect, select the Network tab and filter WS traffic.
3. Follow the [JS SDK Demo App](js-sdk-demo-app.md) instructions to successfully register the demo app.

![Demo app debug view](_images/demo-debug.png)
...
At a high level, when the client is instantiated, it creates a [session object](https://github.com/team-telnyx/webrtc/blob/main/packages/js/src/Modules/Verto/BaseSession.ts#L42) and registers handlers for all [4 socket events](https://github.com/team-telnyx/webrtc/blob/main/packages/js/src/Modules/Verto/BaseSession.ts#L335).

Invoking the `connect` method on the SDK client initiates a WebSocket connection to `rtc.telnyx.com`. Once the socket is open, the [login message](https://github.com/team-telnyx/webrtc/blob/main/packages/js/src/Modules/Verto/index.ts#L63) is sent to `rtc.telnyx.com`. At this point, Messages #1 and #2 are observed.

Messages #3, #4, and #5 are the result of the [VertoHandler logic](https://github.com/team-telnyx/webrtc/blob/main/packages/js/src/Modules/Verto/webrtc/VertoHandler.ts#L120):
...
![WebRTC internals view](_images/webrtc-internals.png)
...
2. [`getUserMedia`](https://developer.mozilla.org/en-US/docs/Web/API/MediaDevices/getUserMedia) is invoked to obtain the user's permission for audio and eventually the [`MediaStream`](https://developer.mozilla.org/en-US/docs/Web/API/MediaStream).
...
6. This API call will eventually create an [`RTCSessionDescription`](https://developer.mozilla.org/en-US/docs/Web/API/RTCSessionDescription) with information on the local media stream.
...
7. The SDK then invokes [`setLocalDescription`](https://developer.mozilla.org/en-US/docs/Web/API/RTCPeerConnection/setLocalDescription) to set the SDP of the client peer.
...
9. When all is done, the `icecandidate` event triggers with `candidate = null` to indicate the process is completed. Subsequently, the existing local SDP parameters are augmented with the ICE candidates.
10. At this point, all necessary info is present to send the invite to `rtc.telnyx.com`.
11. As a result, on the WebSocket, Messages #1 through #4 are observed.
